  ...
  <div class="app-layout">
    <aside class="sidebar" :class="{ collapsed: isCollapsed }">
      ...
      <div class="sidebar-role" v-show="!isCollapsed">
        <el-tag type="warning" effect="dark" size="small" class="role-badge">
      ...
      <div class="sidebar-footer">
        <div class="user-mini" v-show="!isCollapsed">
          ...
          <div class="user-mini-info">
            ...
            <div class="user-mini-role">导师</div>
          </div>
        </div>
...
import {
  User, Document, Edit, MapLocation, Collection,
  School, Fold, Expand, SwitchButton,
...
const menuItems = [
  { path: '/teacher/students', label: '学生管理', icon: User },
  { path: '/teacher/internships', label: '实习审核', icon: Document },
  { path: '/teacher/reports', label: '汇报批阅', icon: Edit },
  { path: '/teacher/locations', label: '签到记录', icon: MapLocation },
]
...
const pageTitle = computed(() => {
  if (route.path.startsWith('/teacher/students')) return '学生管理'
  if (route.path.startsWith('/teacher/internships')) return '实习审核'
  if (route.path.startsWith('/teacher/reports')) return '汇报批阅'
  if (route.path.startsWith('/teacher/locations')) return '签到记录'
  return '导师端'
})

const currentPageIcon = computed(() => {
  if (route.path.startsWith('/teacher/students')) return User
  if (route.path.startsWith('/teacher/internships')) return Document
  if (route.path.startsWith('/teacher/reports')) return Edit
  if (route.path.startsWith('/teacher/locations')) return MapLocation
  return User
})
...
.nav-active-indicator {
  ...
  border-radius: 0 3px 3px 0; opacity: 0; transition: opacity var(--transition-fast);
}
